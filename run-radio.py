...
try:
    import alsaaudio
    alsa_lib_exists = True
except ImportError:
    alsa_lib_exists = False
    pass

# force to not use alsa volume control yet
alsa_lib_exists = False
...
class Interface:
    ...
    def send_init(self):
#        self.serial.write('M:' + str(self.max_value) + "\r\n")
#        debug("write: " + 'M:' + str(self.max_value))
#        time.sleep(Config.write_delay)
#        self.serial.write('E:' + str(self.encoder) + "\r\n")
#        debug("write: " + 'E:' + str(self.encoder))
#        time.sleep(Config.write_delay)
#        i = 0
#        for station in self.stations:
#            i += 1
#            self.serial.write('S' + str(i) + ':' + station.name.upper() + "\r\n")
#            debug("write: " + 'S' + str(i) + ':' + station.name.upper())
#            time.sleep(Config.write_delay)
	self.serial.write("\r\n");
	time.sleep(Config.write_delay);
	self.serial.write("\r\n");
    ...
    def try_serial(self):
        for serial_dev in Config.serial_dev:
            if not self.serial_connected:
                try:
                    # try to reset first
                    # self.serial = serial.Serial(serial_dev, 1200)
...
class Program:
    ...
    def begin(self):
        # init alsa mixer
        if alsa_lib_exists:
            self.mixer = alsaaudio.Mixer(Config.mixer_channel, 0, Config.sound_card)
        ...
        if self.mixer is not None:
            self.mixer.setvolume(self.interface.volume)
    ...
    def begin(self, program):
        ...
        while True:
            ...
            if self.program.interface.volume != self.program.active_volume:
                ...
                if self.program.mixer is not None:
                    self.program.mixer.setvolume(self.program.active_volume)
